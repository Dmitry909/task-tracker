...
def test_signup_login_update():
    ...
    update_resp = update_user_data(token, {'first_name': 'A', 'phone_number': 'B'})
    assert (update_resp.status_code == 200)
    get_resp = get_user_data(token)
    ...
    update_resp = update_user_data(token, {'second_name': 'C', 'phone_number': 'D'})
    get_resp = get_user_data(token)
    get_dict = json.loads(get_resp.text)
    assert(get_dict['first_name'] == 'A')
